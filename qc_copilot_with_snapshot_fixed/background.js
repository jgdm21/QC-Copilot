...
// You can override this at runtime by setting chrome.storage.local key:
//   qcZendeskProxyUrl = 'https://script.google.com/macros/s/.../exec'
// This avoids hardcoding a URL when the Apps Script deployment changes.
...
async function getZendeskInfoCached(email) {
  ...
  try {
    // Allow overriding the proxy URL without editing the extension code
    let proxyUrl = QC_ZENDESK_PROXY;
    try {
      const { qcZendeskProxyUrl } = await getLocal(['qcZendeskProxyUrl']);
      if (qcZendeskProxyUrl && typeof qcZendeskProxyUrl === 'string' && qcZendeskProxyUrl.startsWith('https://')) {
        proxyUrl = qcZendeskProxyUrl.trim();
      }
    } catch (_) {}

    const url = `${proxyUrl}?email=${encodeURIComponent(email)}`;
    const resp = await fetch(url, { method: 'GET', cache: 'no-store' });
      ...
      chrome.storage.local.get(['releaseData'], ({ releaseData }) => {
        (async () => {
          try {
            ...
            // Build curated list from columns A and C (0 and 2)
            // User confirmed curated artists live in column A and C.
            let curatedList = [];
            try {
              if (Array.isArray(curatedTable) && curatedTable.length > 0) {
                const rows = curatedTable.slice(1);
                const out = [];
                rows.forEach(r => {
                  if (!r) return;
