...
// Data load status tracking (updated by background.js)
let dataLoadStatus = {
  blacklistsLoaded: null,
  blacklistsLoadedAt: null,
  blacklistsError: null,
  curatedArtistsLoaded: null,
  curatedArtistsLoadedAt: null,
  curatedArtistsError: null,
  suspiciousTermsLoaded: null,
  suspiciousTermsLoadedAt: null,
  suspiciousTermsError: null,
  spotifyDataLoaded: null,
  spotifyDataLoadedAt: null,
  spotifyDataError: null,
  spotifyDataCacheUsed: false,
  zendeskDataLoaded: null,
  zendeskDataLoadedAt: null,
  zendeskDataError: null
};
...
// ================================
// Snapshot JSON v1 (for Lovable)
// ================================
function qcSafeStr(v) {
  return (v === null || v === undefined) ? '' : String(v);
}

function qcNormalizeStr(v) {
  return qcSafeStr(v).trim().toLowerCase().replace(/\s+/g, ' ');
}

function qcSplitArtists(v) {
  const s = qcSafeStr(v).trim();
  if (!s) return [];
  return s.split(/\s*,\s*/).map(x => x.trim()).filter(Boolean);
}

function qcParseDurationSeconds(v) {
  const s = qcSafeStr(v).trim();
  if (!s) return null;
...
function qcIsSolidLanguageFlag(lang) {
  const l = qcNormalizeStr(lang);
  return l.includes('turkish') || l.includes('vietnamese') || l === 'tr' || l === 'vi';
}

function qcDetectComposerFormatInvalid(credits) {
  // Deterministic minimal check: every person should have at least 2 tokens.
  // If the field is empty, do not flag.
  const arr = qcSplitArtists(credits);
...
function qcComputePrecomputedFlags() {
  const rd = currentState.releaseData || {};
  const flagsObj = currentState.flags || {};
  const out = new Set();

  // Release previously rejected
  if (currentState.previouslyRejected) out.add('FLAG_PREVIOUSLY_REJECTED_RELEASE');

  // Version tags
  try {
    const releaseVersion = rd.basicInfo?.Metadata?.Version || rd.basicInfo?.Metadata?.['Release Version'] || '';
    if (qcSafeStr(releaseVersion).trim()) out.add('FLAG_RELEASE_VERSION_PRESENT');
  } catch (_) {}

  // Release language
  try {
    const relLang = rd.basicInfo?.Metadata?.Language || '';
    if (qcIsSolidLanguageFlag(relLang)) out.add('FLAG_LANGUAGE_RARE_RELEASE');
  } catch (_) {}

  // Explicit at release
  try {
    const explicitVal = qcNormalizeStr(rd.basicInfo?.Metadata?.Explicit || '');
    if (explicitVal === 'explicit' || currentState.explicitFound) out.add('FLAG_EXPLICIT_CONTENT_DETECTED');
  } catch (_) {}

  // User context
  try {
    const strikes = Array.isArray(rd.userStrikes) ? rd.userStrikes : [];
    if (strikes.length > 0) out.add('FLAG_USER_HAS_STRIKES');
  } catch (_) {}
  try {
    const ver = qcNormalizeStr(rd.userVerificationStatus || '');
    if (ver && !ver.includes('verified')) out.add('FLAG_USER_NOT_VERIFIED');
  } catch (_) {}
  if (currentState.suspiciousEmailDomain) out.add('FLAG_SUSPICIOUS_EMAIL_DOMAIN');

  // Zendesk
  try {
    const z = currentState.zendeskInfo;
    if (z && Number(z.ticketCount || 0) > 0) out.add('FLAG_USER_HAS_ZENDESK_TICKETS');
    if (z && Number(z.fraudTickets || 0) > 0) out.add('FLAG_USER_FRAUD_TICKET_HISTORY');
  } catch (_) {}

  // Sheets derived
  if (Array.isArray(flagsObj.blacklistEmails) && flagsObj.blacklistEmails.length) out.add('FLAG_BLACKLISTED_EMAIL');
  if (Array.isArray(flagsObj.blacklistArtists) && flagsObj.blacklistArtists.length) out.add('FLAG_BLACKLISTED_ARTIST');
  if (Array.isArray(flagsObj.blacklistLabels) && flagsObj.blacklistLabels.length) out.add('FLAG_BLACKLISTED_LABEL');
  if (Array.isArray(flagsObj.curatedArtists) && flagsObj.curatedArtists.length) out.add('FLAG_ARTIST_MATCHES_CURATED_ARTIST');
  if (Array.isArray(flagsObj.terms) && flagsObj.terms.length) out.add('FLAG_SUSPICIOUS_TERMS');

  // Tracks
  const tracks = Array.isArray(rd.trackSections) ? rd.trackSections : [];
  if (tracks.length === 0) out.add('FLAG_LOW_CONFIDENCE');

  // Track versions, languages and durations
  let shortCount = 0;
  let durationCount = 0;
  let clusterCount = 0;
  ...
  let hasLong = false;
  let hasShort = false;
  let hasTrackVersion = false;
  let hasTrackLangRare = false;
  const trackTitles = tracks.map(t => qcSafeStr(t.header).trim()).filter(Boolean);
  const cleanedTitles = trackTitles.map(t => t.replace(/^\s*\d+[).\-:]\s*/, '').trim().toLowerCase());
  const sortedTitles = [...cleanedTitles].sort();
  if (cleanedTitles.length > 2 && cleanedTitles.join('||') === sortedTitles.join('||')) out.add('FLAG_TRACKLIST_ALPHABETICAL');

  tracks.forEach(t => {
    const md = t?.sections?.Metadata || {};
    if (qcSafeStr(md.Version || md['Track Version'] || '').trim()) hasTrackVersion = true;
    const lang = md.Language || md['Track Language'] || '';
    if (qcIsSolidLanguageFlag(lang)) hasTrackLangRare = true;

    const dStr = qcSafeStr(t.duration).trim();
    const sec = qcParseDurationSeconds(dStr);
    if (sec !== null) {
      durationCount++;
      if (sec >= 1200) hasLong = true;
      if (sec <= 30) hasShort = true;
      if (sec >= 60 && sec <= 119) clusterCount++;
      if (sec === 0) zeroDuration = true;
    } else {
      if (dStr === '00:00:00') zeroDuration = true;
    }
  });
  if (hasTrackVersion) out.add('FLAG_TRACK_VERSION_PRESENT');
  if (hasTrackLangRare) out.add('FLAG_LANGUAGE_RARE_TRACK');
  if (hasLong) out.add('FLAG_TRACK_DURATION_LONG');
  if (hasShort) out.add('FLAG_TRACK_DURATION_SHORT');
  if (zeroDuration) out.add('FLAG_AUDIO_MISSING_OR_INVALID');
  if (durationCount >= 3 && (clusterCount / durationCount) >= 0.8) out.add('FLAG_TRACK_DURATION_CLUSTER_1_59');

  // Potential mashups
  if (Array.isArray(currentState.potentialMashups) && currentState.potentialMashups.length) out.add('FLAG_POTENTIAL_MASHUP');

  // Composer/Lyricist format
  try {
    const comp = rd.basicInfo?.Artists?.Composer || '';
    const lyr = rd.basicInfo?.Artists?.Lyricist || '';
    if (qcDetectComposerFormatInvalid(comp)) out.add('FLAG_COMPOSER_NAME_FORMAT_INVALID');
    if (qcDetectComposerFormatInvalid(lyr)) out.add('FLAG_LYRICIST_NAME_FORMAT_INVALID');
  } catch (_) {}

  // Top songs matches (Spotify JSON already loaded in drawer)
  try {
    const spotifyTracks = Array.isArray(currentState.spotifyTracks) ? currentState.spotifyTracks : [];
    if (spotifyTracks.length) {
      const relMatch = findTitleMatch(rd.title, spotifyTracks);
      if (relMatch) out.add('FLAG_RELEASE_TITLE_MATCHES_TOP_SONG');
      const anyTrackMatch = tracks.some(t => !!findTitleMatch(t.header, spotifyTracks));
      if (anyTrackMatch) out.add('FLAG_TRACK_TITLE_MATCHES_TOP_SONG');
    }
  } catch (_) {}

  // Audio analysis results (ACRCloud modal)
  try {
    const ars = Array.isArray(currentState.audioAnalysisResults) ? currentState.audioAnalysisResults : [];
    if (ars.length) out.add('FLAG_AUDIO_MATCH_FOUND');
    let any100 = false;
    let anySuspicious = false;
    let titleHigh = false;
    let titleMed = false;
    let artistHigh = false;
    let artistMed = false;
    let artistMismatch = false;
    let titleMismatch = false;

    const relArtist = qcNormalizeStr(rd.basicInfo?.Artists?.['Primary Artist'] || '');

    ars.forEach(a => {
      const results = Array.isArray(a.results) ? a.results : [];
      results.forEach(r => {
        if (Number(r.score) === 100) any100 = true;
        const alerts = Array.isArray(r.alerts) ? r.alerts : [];
        alerts.forEach(al => {
          if (al.type === 'title_similar') titleHigh = true;
          if (al.type === 'title_moderately_similar') titleMed = true;
          if (al.type === 'artist_similar') artistHigh = true;
          if (al.type === 'artist_moderately_similar') artistMed = true;
          if (al.type === 'artist_different') artistMismatch = true;
          if (al.type === 'title_different') titleMismatch = true;
        });
        // Direct mismatch check vs release primary artist (if available)
        if (relArtist && Array.isArray(r.artists) && r.artists.length) {
          const anySame = r.artists.some(x => qcNormalizeStr(x) === relArtist);
          if (!anySame) artistMismatch = true;
        }
      });
    });

    if (any100) out.add('FLAG_AUDIO_MATCH_EXACT');
    if (anySuspicious) out.add('FLAG_AUDIO_MATCH_SUSPICIOUS');
    if (titleHigh) out.add('FLAG_AUDIO_TITLE_SIMILARITY_HIGH');
    if (titleMed) out.add('FLAG_AUDIO_TITLE_SIMILARITY_MEDIUM');
    if (artistHigh) out.add('FLAG_AUDIO_ARTIST_SIMILARITY_HIGH');
    if (artistMed) out.add('FLAG_AUDIO_ARTIST_SIMILARITY_MEDIUM');
    if (artistMismatch) out.add('FLAG_AUDIO_ARTIST_MISMATCH_WITH_RELEASE');
    if (titleMismatch) out.add('FLAG_AUDIO_TITLE_MISMATCH');
  } catch (_) {}

  return Array.from(out);
}

async function qcSha256Hex(text) {
...
function qcCopyToClipboard(text) {
...
function qcShowSnapshotCopyModal(text) {
  // Create a simple modal with a textarea so the user can copy manually if clipboard APIs fail
  try {
    // Remove existing modal if present
    const existing = document.getElementById('qc-snapshot-copy-modal');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.id = 'qc-snapshot-copy-modal';
    ...
    title.textContent = 'Copy snapshot JSON';
    title.style.fontWeight = '700';
    title.style.marginBottom = '6px';

    const hint = document.createElement('div');
    hint.textContent = 'Clipboard write is blocked here. The snapshot is shown below. Use Ctrl+C after selecting.';
    ...
    console.error('Failed to show snapshot modal', e);
  }
}


async function qcBuildSnapshotV1() {
  const rd = currentState.releaseData || {};
  const flagsObj = currentState.flags || {};
  const precomputed_flags = qcComputePrecomputedFlags();

  // Release ID from URL - usar currentState en lugar de window.parent.location
  const href = currentState.releaseUrl || '';
  const releaseId = (href.match(/\/quality-control\/revisions\/(\d+)/) || [])[1] || '';

  // Release fields
  const primaryArtist = qcSplitArtists(rd.basicInfo?.Artists?.['Primary Artist'] || '')[0] || qcSafeStr(rd.basicInfo?.Artists?.['Primary Artist'] || '').trim();
  const featured = qcSplitArtists(rd.basicInfo?.Artists?.['Featured Artist'] || '');
  const remixer = qcSplitArtists(rd.basicInfo?.Artists?.Remixer || '');
  const additionalArtists = Array.from(new Set([...featured, ...remixer].filter(Boolean)));

  const releaseLangRaw = rd.basicInfo?.Metadata?.Language || '';
  const explicitVal = qcNormalizeStr(rd.basicInfo?.Metadata?.Explicit || '');
  const releaseExplicit = (explicitVal === 'explicit') || !!currentState.explicitFound;

  const releaseObj = {
    release_id: releaseId || '',
    title: qcSafeStr(rd.title).trim(),
    primary_artist: qcSafeStr(primaryArtist).trim(),
    additional_artists: additionalArtists,
    label: qcSafeStr(rd.basicInfo?.Metadata?.Label || rd.cards?.Label || '').trim(),
    upc: qcSafeStr(rd.basicInfo?.Metadata?.UPC || '').trim(),
    release_version: qcSafeStr(rd.basicInfo?.Metadata?.Version || '').trim() || null,
    release_date: qcSafeStr(rd.cards?.['Release Date'] || '').trim() || null,
    language: qcSafeStr(releaseLangRaw).trim() || null,
    explicit: releaseExplicit,
    territories: ['WORLD'],
    previously_rejected: !!currentState.previouslyRejected
  };

  // Tracks
  const tracks = (Array.isArray(rd.trackSections) ? rd.trackSections : []).map((t, idx) => {
    const md = t?.sections?.Metadata || {};
    const artists = t?.sections?.Artists || {};
    const tPrimary = qcSplitArtists(artists['Primary Artist'] || '')[0] || qcSafeStr(artists['Primary Artist'] || '').trim();
    const tAdd = Array.from(new Set([
      ...qcSplitArtists(artists['Featured Artist'] || ''),
      ...qcSplitArtists(artists.Remixer || '')
    ].filter(Boolean)));
    const durSec = qcParseDurationSeconds(t.duration);
    return {
      track_id: qcSafeStr(t.trackId || t.id || '').trim() || `idx_${idx}`,
      title: qcSafeStr(t.header).trim(),
      version: qcSafeStr(md.Version || md['Track Version'] || '').trim() || null,
      primary_artist: qcSafeStr(tPrimary).trim(),
      additional_artists: tAdd,
      duration_seconds: durSec,
      explicit: releaseExplicit,
      language: qcSafeStr(md.Language || md['Track Language'] || '').trim() || null,
      isrc: qcSafeStr(md.ISRC || '').trim() || null
    };
  });

  // User context
  const email = qcSafeStr(rd.cards?.User?.Email || '').trim().toLowerCase();
  const z = currentState.zendeskInfo || null;
  const strikes = Array.isArray(rd.userStrikes) ? rd.userStrikes : [];
  const userContext = {
    user_id: qcSafeStr(rd.cards?.User?.Id || '').trim() || null,
    email: email || null,
    verified: qcNormalizeStr(rd.userVerificationStatus || '').includes('verified'),
    has_strikes: strikes.length > 0,
    strike_levels: strikes,
    fraud_ticket_count: z ? Number(z.fraudTickets || 0) : null,
    suspicious_email_domain: !!currentState.suspiciousEmailDomain,
    blacklists: {
      email: Array.isArray(flagsObj.blacklistEmails) && flagsObj.blacklistEmails.length > 0,
      artist: Array.isArray(flagsObj.blacklistArtists) && flagsObj.blacklistArtists.length > 0,
      label: Array.isArray(flagsObj.blacklistLabels) && flagsObj.blacklistLabels.length > 0
    },
    zendesk: z ? {
      status: z.status || null,
      total_tickets: Number(z.ticketCount || 0),
      fraud_tickets: Number(z.fraudTickets || 0),
      last_updated: z.lastUpdated || null
    } : null
  };

  // Audio analysis (ACRCloud modal rendered)
  const audioAnalysis = {
    source: 'acrcloud_modal',
    per_track: (Array.isArray(currentState.audioAnalysisResults) ? currentState.audioAnalysisResults : []).map((a, idx) => {
      const results = Array.isArray(a.results) ? a.results : [];
      const scores = results.map(r => Number(r.score || 0));
      const maxScore = scores.length ? Math.max(...scores) : 0;
      const exactCount = scores.filter(s => s === 100).length;
      const top = results.slice(0, 3).map(r => ({
        artist: (Array.isArray(r.artists) ? r.artists.join(', ') : qcSafeStr(r.artist)).trim(),
        album: qcSafeStr(r.album).trim(),
        track: qcSafeStr(r.title).trim(),
        score_percent: Number(r.score || 0)
      }));
      return {
        track_id: qcSafeStr(a.trackId || '').trim() || `audio_idx_${idx}`,
        track_title: qcSafeStr(a.trackTitle).trim(),
        fragments_total: Number(a.fragmentsTotal || a.fragments || 0) || null,
        results_total: results.length,
        exact_match_count: exactCount,
        max_score_percent: maxScore,
        top_matches: top
      };
    })
  };
  ...
  const assets = {
    cover_art: {
      url: qcSafeStr(rd.coverUrl).trim() || null,
      present: !!qcSafeStr(rd.coverUrl).trim(),
      solid_color_detected: precomputed_flags.includes('FLAG_IMAGE_SOLID_COLOR')
    }
  };

  // Sheet signals (minimal)
  const sheetSignals = {
    curated_artist_match: {
      matched: Array.isArray(flagsObj.curatedArtists) && flagsObj.curatedArtists.length > 0,
      matched_values: (flagsObj.curatedArtists || []).slice(0, 10),
      source_gid: 573812010,
      matched_on_columns: ['A', 'C']
    },
    blacklist: {
      email: { matched: Array.isArray(flagsObj.blacklistEmails) && flagsObj.blacklistEmails.length > 0, source_gid: 137283885 },
      artist: { matched: Array.isArray(flagsObj.blacklistArtists) && flagsObj.blacklistArtists.length > 0, source_gid: 0 },
      label: { matched: Array.isArray(flagsObj.blacklistLabels) && flagsObj.blacklistLabels.length > 0, source_gid: 55458291 }
    },
    suspicious_terms: {
      matched: Array.isArray(flagsObj.terms) && flagsObj.terms.length > 0,
      matched_terms: (flagsObj.terms || []).slice(0, 25),
      source_gid: 923800218
    }
  };
  ...
  const snapshot = {
    ...
    precomputed_flags,
    sheet_signals: sheetSignals,
    historical_signals: {
      recurrence_triggered: !!(currentState.tenantInfo && currentState.tenantInfo.consideration && qcNormalizeStr(currentState.tenantInfo.consideration).includes('repeat'))
    },
    context_metadata: {
      copilot_version: (currentState.context_metadata?.copilot_version || 'qc_copilot_clipboard_updated'),
      tenant_code: qcSafeStr(rd.tenantCode).trim() || null,
      tenant_name: qcSafeStr(rd.tenantName).trim() || null
    }
  };

  // Deterministic hash over the snapshot without the hash field
  const clone = JSON.parse(JSON.stringify(snapshot));
  clone.snapshot_hash = '';
  const json = JSON.stringify(clone);
  snapshot.snapshot_hash = await qcSha256Hex(json);
  return snapshot;
}

async function qcCopySnapshot() {
  const btn = document.getElementById('qc-copy-snapshot');
  if (btn) btn.disabled = true;
  try {
    const snapshot = await qcBuildSnapshotV1();
    const text = JSON.stringify(snapshot, null, 2);

    try {
      await qcCopyToClipboard(text);
      showSuccessMessage('Snapshot copied');
    } catch (copyErr) {
      console.warn('Clipboard write blocked, showing modal instead', copyErr);
      qcShowSnapshotCopyModal(text);
      showSuccessMessage('Snapshot ready to copy');
    }
  } catch (e) {
    console.error('Snapshot build failed', e);
    showErrorMessage('Snapshot build failed');
  } finally {
...
function updateUI(newData) {
  ...
  // NUEVO: Update data load status if provided
  if (newData.dataLoadStatus) {
    dataLoadStatus = { ...dataLoadStatus, ...newData.dataLoadStatus };
  }
